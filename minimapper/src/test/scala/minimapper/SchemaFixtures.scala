...
object SchemaFixtures {
  ...
  val QualitativeTurbiditySchema =
    ProductSchema(ListMap("description" -> StringSchema))

  val QuantitativeTurbiditySchema =
    ProductSchema(ListMap("value" -> IntSchema))

  val TurbiditySchema = SumSchema(ListMap(
    "QuantitativeTurbidity" -> QuantitativeTurbiditySchema,
    "QualitativeTurbidity"  -> QualitativeTurbiditySchema
  ))

  val WaterQualitySchema = ProductSchema(ListMap(
    "location"    -> LocationSchema,
    "timestamp"   -> TimestampSchema,
    "river"       -> OptionSchema(StringSchema),
    "temperature" -> OptionSchema(DoubleSchema),
